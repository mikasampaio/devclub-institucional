...
export default function Projects() {
  ...
  return (
    <ParallaxProvider isDisabled={!!reduceMotion}>
      <section id="project" className="mx-auto max-w-6xl px-5 py-24 sm:py-32">
        <SectionHeading
          badge="Projetos dos alunos"
          title={<>Projetos reais criados por alunos</>}
        />

        <div className="mt-14 grid grid-cols-3 items-start gap-6">
          {PROJECTS.map((project, i) => (
            <Parallax key={project.alt} speed={i % 2 ? -5 : 10}>
              <Reveal delay={i * 0.08}>
